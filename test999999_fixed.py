...
def put_chinese_text(img, text, position, text_color=(0, 0, 255), font_size=30):
    # 将OpenCV图像转换为PIL图像
    ...
    try:
        ...
        font_paths = [
            "simhei.ttf",  # 当前目录
            "C:\\Windows\\Fonts\\simhei.ttf",  # Windows系统字体目录
            "/usr/share/fonts/truetype/simhei.ttf"  # Linux系统字体目录
        ]
...
def ORBMatcher(img1, img2):
    ...
    if circles is not None:
        ...
        for i in circles[0, :]:
            ...
            if (result_red):
                ...
                # 使用PIL绘制中文文本
                img2 = put_chinese_text(img2, "circle", (200, 30), (0, 0, 255), 30)
            # 如果检测到蓝色区域，绘制蓝色圆
            if (result_blue):
                cv2.circle(img2, center, radius, (255, 0, 0), 3)
                # 使用PIL绘制中文文本
                img2 = put_chinese_text(img2, "circle", (200, 35), (255, 0, 0), 30)
    ...
    if len(points_good) > MIN_MATCH_COUNT:
        ...
        if (result_red):
            img3 = put_chinese_text(img3, "颜色:红色", (400, 120), (0, 0, 255), 30)
        # 如果检测到蓝色区域，在图像上显示蓝色文字
        if (result_blue):
            img3 = put_chinese_text(img3, "颜色:蓝色", (400, 120), (255, 0, 0), 30)
        # 在图像上显示旋转角度和误差值
        img3 = put_chinese_text(img3, f"angle:{int(theta)}", (210, 70), (128, 0, 128), 30)
        img3 = put_chinese_text(img3, f"error:{error[-1]}mm", (500, 70), (128, 0, 128), 30)
        # 显示匹配结果图像
